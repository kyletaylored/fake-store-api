...
## Five more things fixed by actually running the deploy

Real `gcloud builds submit --config=cloudbuild.yaml` runs against a live project/`us-west1` surfaced five more Cloud Run/Cloud Build specifics, now fixed in the committed files:

1. **`$SHORT_SHA` is empty on a manual `gcloud builds submit`** - it's a built-in substitution only populated for trigger-based builds tied to a repo commit. Switched the secondary image tag to `$BUILD_ID`, which is always available.
2. **Built-in substitutions aren't auto-injected into `script:` step containers as shell env vars** - `$BUILD_ID` referenced directly inside a `script:` block resolved to empty, because Cloud Build only text-substitutes it into config-level fields (like `env:` list *values*, or `args:`), not into script content at runtime. Fixed by explicitly passing `BUILD_ID=$BUILD_ID` via each step's `env:` list, then referencing `${BUILD_ID}` in the script as a real shell variable.
3. **Cloud Run rejects two things `cloud-run/service.yaml` was doing**: an explicit `PORT` env var (reserved - Cloud Run sets it automatically from `containerPort`, and errors if you also set it yourself), and a container dependency (`datadog-agent`, via `run.googleapis.com/container-dependencies`) with no `startupProbe` - Cloud Run requires one on every container something else depends on, so a `tcpSocket` probe on port 8126 (the trace-agent port) was added. Separately, the Datadog Agent image's own startup script hard-exits if `DD_API_KEY` is empty - since `app` depends on `datadog-agent`, an empty key blocked the whole revision from ever becoming ready. `DD_API_KEY` now defaults to the literal placeholder `REPLACE_ME` (unblocks deploys, sends no real telemetry) instead of an empty string.
4. **`ejs` was in `devDependencies`**, but it's the actual runtime view engine for the home page (`app.js` sets it as `view engine`) - `Dockerfile` runs `npm ci --omit=dev`, so production images never had it, and only the `/` route (not the JSON API routes) ever hit `Cannot find module 'ejs'`. Moved to `dependencies`.
5. **Deploying against `:latest` silently no-ops on redeploy** - Cloud Run's `services replace` skips creating a new revision when the submitted YAML text is byte-for-byte unchanged, and `image: ...app:latest` is the same string on every run even though the tag moves to a new digest each time. Two "successful" deploys in a row kept serving the *first* image. Both `cloudbuild.yaml` and `deploy.sh` now render the service YAML with a unique per-build tag (`$BUILD_ID`, or the git short SHA for `deploy.sh`) instead of `:latest`, guaranteeing the spec text actually changes.

**Separately, worth knowing but not blocking:** the Datadog Agent needs `DD_HOSTNAME` set explicitly on Cloud Run - without it, hostname auto-detection fatally errors (`unable to reliably determine the host name`), which crash-loops the whole `datadog-agent` container. Fixed with a static `DD_HOSTNAME` value in `cloud-run/service.yaml`.
